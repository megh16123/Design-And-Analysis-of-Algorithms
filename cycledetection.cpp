...
void cycle_det(vector<vector<int>> &adj_list, vector<bool> &visited, int curr_node, vector<bool> &in_path, bool &isFound){
	if(isFound) return;
	...
	for(auto i: adj_list[curr_node]){
		if(!visited[i]){
			cycle_det(adj_list,visited,i,in_path,isFound);
		}else{
			if(isFound) return;
...
int main(){
	...
	for(int i=0;i<num_edges;i++){
		...
		int v1, v2;	cin>>v1>>v2;
		start_node = min(start_node,v1);
		
		adj_list[v1].push_back(v2);
	}
	
	vector<bool> visited(num_ver+1,false);	bool isFound = false;
	vector<bool> in_path(num_ver+1,false);
	cycle_det(adj_list,visited,start_node,in_path,isFound);
